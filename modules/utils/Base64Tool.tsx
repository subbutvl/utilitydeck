    ...
    <div className="h-full flex flex-col gap-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1">
        <div className="flex flex-col gap-2">
          <label className="text-[10px] uppercase text-[var(--text-muted)] font-bold tracking-widest">Raw Input</label>
          <textarea 
            value={input}
            onChange={(e) => setInput(e.target.value)}
            className="flex-1 bg-[var(--bg-card)] border border-[var(--border)] p-4 font-mono text-xs text-[var(--text-primary)] focus:outline-none focus:border-[var(--border-hover)] resize-none"
            placeholder="Enter text here..."
          />
        </div>
        <div className="flex flex-col gap-2">
          <label className="text-[10px] uppercase text-[var(--text-muted)] font-bold tracking-widest">Result Output</label>
          <div className="flex-1 relative">
            ...
            {output && (
              <button 
                ...
                className="absolute top-2 right-2 px-3 py-1 bg-[var(--bg-hover)] border border-[var(--border)] text-[9px] uppercase tracking-widest"
              >
                Copy
              </button>
            )}
