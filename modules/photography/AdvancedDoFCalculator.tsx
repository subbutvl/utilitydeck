  ...
  return (
    <div className="h-full flex flex-col gap-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 min-h-0">
        <div className="lg:col-span-4 bg-[#111] border border-neutral-800 p-8 space-y-6 overflow-y-auto">
          <span className="text-[10px] uppercase text-neutral-500 font-bold tracking-widest block border-b border-neutral-800 pb-2">Optical Bench</span>
        ...
        <div className="lg:col-span-8 bg-[#0a0a0a] border border-neutral-800 p-12 flex flex-col gap-12 justify-center">
           ...
           <div className="bg-[#111] border border-neutral-800 p-8 flex items-center justify-between">
              <div>
                 <div className="text-[10px] uppercase font-bold text-neutral-500 tracking-widest mb-1">Bokeh Analysis</div>
                 <div className="text-[11px] text-neutral-600 uppercase italic">Blur Circle Diameter at {bgDist}m</div>
              </div>
              <div className="text-right">
                 <div className="text-6xl font-mono text-white">{stats.blurSize.toFixed(2)}<span className="text-lg">mm</span></div>
...
const AdvancedStat = ({ label, val }: { label: string; val: string }) => (
  <div className="text-center">
    <div className="text-[10px] uppercase font-bold text-neutral-700 tracking-widest mb-2">{label}</div>
    <div className="text-2xl font-mono text-neutral-300">{val}</div>
  </div>
);
...
const Control = ({ label, value, min, max, step = 1, onChange, unit }: { label: string; value: number; min: number; max: number; step?: number; onChange: (v: number) => void; unit: string }) => (
  <div className="space-y-2">
    <label className="text-[10px] uppercase text-neutral-600 font-bold tracking-widest flex justify-between">{label} <span>{value}{unit}</span></label>
