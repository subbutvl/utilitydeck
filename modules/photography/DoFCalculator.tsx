    ...
    <div className="h-full flex flex-col gap-6">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 flex-1 min-h-0">
        <div className="lg:col-span-4 bg-[var(--bg-card)] border border-[var(--border)] p-8 space-y-6">
          <span className="text-[10px] uppercase text-[var(--text-muted)] font-bold tracking-widest block border-b border-[var(--border)] pb-2">Inputs</span>
          ...
          <div className="space-y-6">
             ...
             <div className="space-y-2">
                <label className="text-[10px] uppercase text-[var(--text-muted)] font-bold tracking-widest block">Sensor / CoC Preset</label>
                <select value={sensor} onChange={(e) => setSensor(parseFloat(e.target.value))} className="w-full bg-[var(--bg-input)] border border-[var(--border)] p-3 text-sm text-[var(--text-primary)] outline-none">
          ...
          <button onClick={() => {setFocalLength(50); setAperture(2.8); setDistance(2); setSensor(0.03);}} className="w-full py-2 text-[10px] uppercase text-[var(--text-muted)] hover:text-red-500 font-bold">Reset Optics</button>
        </div>

        <div className="lg:col-span-8 flex flex-col gap-6">
          <div className="flex-1 bg-[var(--bg-page)] border border-[var(--border)] p-12 overflow-y-auto">
...
const Control = ({ label, value, min, max, step = 1, onChange }: { label: string; value: number; min: number; max: number; step?: number; onChange: (v: number) => void }) => (
  <div className="space-y-2">
    <label className="text-[10px] uppercase text-[var(--text-muted)] font-bold tracking-widest flex justify-between">{label} <span>{value}</span></label>
...
const ResultItem = ({ label, value, highlight = false }: { label: string; value: string; highlight?: boolean }) => (
  <div className={`p-8 border border-[var(--border)] flex flex-col items-center justify-center text-center ${highlight ? 'bg-white/5 border-white/10' : ''}`}>
    <div className="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-widest mb-4">{label}</div>
    <div className={`font-mono leading-none ${highlight ? 'text-[var(--text-primary)] text-6xl' : 'text-[var(--text-muted)] text-4xl'}`}>{value}</div>
  </div>
);
