      ...
      <div className="flex-1 overflow-auto space-y-8 pb-20">
        {loading ? (
          <div className="h-64 animate-pulse bg-[var(--bg-card)] border border-[var(--border)] flex items-center justify-center">
             <span className="text-[10px] uppercase tracking-[0.4em] text-[var(--text-muted)] font-bold">Forging Vocabulary...</span>
          </div>
        ) : primary ? (
          <section className="bg-[#0d0d0d] border border-[var(--border)] p-10 relative overflow-hidden group">
            <div className="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-100 transition-opacity">
               <svg xmlns="http://www.w3.org/2000/svg" width="48" height="48" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1" className="text-[var(--text-primary)]"><polyline points="4 7 4 4 20 4 20 7"/><line x1="9" x2="15" y1="20" y2="20"/><line x1="12" x2="12" y1="4" y2="20"/></svg>
            </div>
            <div className="max-w-3xl space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-[10px] text-[var(--text-muted)] font-mono">{primary.phonetic}</span>
              </div>
              <h2 className="text-5xl font-bold text-[var(--text-primary)] tracking-tighter">{primary.word}</h2>
              <p className="text-lg text-[var(--text-muted)] leading-relaxed font-light">{primary.definition}</p>
              <p className="text-sm text-[var(--text-muted)] leading-relaxed italic border-l-2 border-[var(--border)] pl-4 mt-4">"{primary.example}"</p>
            </div>
          </section>
        ) : null}
          ...
          {secondary.map((w, i) => (
            <div key={i} className="bg-[var(--bg-card)] border border-[var(--border)] p-6 space-y-3 hover:border-[var(--border-hover)] transition-colors">
              <div className="flex justify-between items-start">
                <span className="text-[9px] text-[var(--text-muted)] font-bold uppercase tracking-widest">{w.phonetic}</span>
                <span className="text-[9px] text-[var(--text-muted)] font-mono">/ word</span>
              </div>
              <h3 className="text-base font-bold text-[var(--text-primary)]">{w.word}</h3>
              <p className="text-xs text-[var(--text-muted)] line-clamp-2">{w.definition}</p>
            </div>
          ))}
        </div>
      </div>
