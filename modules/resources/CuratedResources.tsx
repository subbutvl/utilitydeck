...
export const CuratedResources: React.FC = () => {
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center space-y-4">
           ...
           <p className="text-[10px] uppercase font-bold text-[var(--text-muted)] tracking-[0.4em]">Establishing Cloud Link...</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div className="h-full flex items-center justify-center p-10">
        <div className="max-w-md text-center space-y-6">
           ...
           <p className="text-xs text-[var(--text-muted)] leading-relaxed font-bold uppercase tracking-widest">{error}</p>
  ...
  return (
    <div className="h-full flex flex-col gap-6">
      ...
      <div className="bg-[var(--bg-card)] border border-[var(--border)] p-6 flex flex-col md:flex-row items-center justify-between gap-6">
        <div className="relative flex-1 w-full max-w-2xl">
          <input 
            ...
            className="w-full bg-[var(--bg-input)] border border-[var(--border)] p-3.5 pl-12 text-sm text-[var(--text-primary)] focus:border-indigo-500 outline-none transition-all placeholder:text-[var(--text-muted)]"
          />
        ...
        <div className="flex items-center gap-6 shrink-0">
           <div className="flex bg-[var(--bg-input)] border border-[var(--border)] p-1">
              <button 
                ...
                className={`p-2 transition-all ${viewMode === 'grid' ? 'bg-[var(--bg-hover)] text-[var(--text-primary)] shadow-lg' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
              >
              ...
              <button 
                ...
                className={`p-2 transition-all ${viewMode === 'table' ? 'bg-[var(--bg-hover)] text-[var(--text-primary)] shadow-lg' : 'text-[var(--text-muted)] hover:text-[var(--text-primary)]'}`}
              >
           ...
           <button 
            ...
            className="text-[10px] uppercase font-bold text-[var(--text-muted)] hover:text-[var(--text-primary)] transition-colors tracking-widest"
           >
             Clear Filters
           </button>
        </div>
      ...
      {/* Domain Navigation */}
      <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar border-b border-[var(--border)]/50">
        {categories.map(cat => (
          <button
            key={cat}
            onClick={() => setActiveCategory(cat)}
            className={`px-5 py-2 border shrink-0 text-[10px] uppercase font-bold tracking-[0.2em] transition-all ${
              activeCategory === cat ? 'bg-indigo-600 text-[var(--text-primary)] border-indigo-600' : 'bg-[var(--bg-card)]/50 text-[var(--text-muted)] border-[var(--border)] hover:border-[var(--border-hover)]'
            }`}
          >
            {cat}
          </button>
      ...
      <div className="flex-1 overflow-y-auto pr-1 pb-24 custom-scrollbar">
        {viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredData.map((item, idx) => (
              <a 
                ...
                className="group p-8 bg-[var(--bg-card)] border border-[var(--border)] hover:border-indigo-500/40 transition-all duration-500 flex flex-col gap-6 relative overflow-hidden"
              >
                ...
                <div className="space-y-2">
                  ...
                  <h3 className="text-lg font-bold text-[var(--text-primary)] group-hover:text-indigo-400 transition-colors tracking-tight leading-tight">{item.name}</h3>
                </div>

                <p className="text-xs text-[var(--text-muted)] leading-relaxed line-clamp-4 font-medium italic opacity-80">
                  {item.description}
                </p>

                <div className="mt-auto pt-6 flex flex-wrap gap-2">
                  {item.tags.map(tag => (
                    <span key={tag} className="text-[9px] text-[var(--text-muted)] bg-[var(--bg-input)]/50 px-2 py-0.5 border border-[var(--border)] font-mono uppercase tracking-tighter group-hover:border-[var(--border)] transition-colors">
                      {tag}
                    </span>
                  ))}
                </div>
          ...
          <div className="bg-[var(--bg-card)] border border-[var(--border)] overflow-hidden shadow-2xl">
            <table className="w-full text-left border-collapse">
              <thead className="bg-[var(--bg-input)] border-b border-[var(--border)]">
                <tr className="text-[10px] uppercase font-black text-[var(--text-muted)] tracking-[0.3em]">
              ...
              <tbody className="divide-y divide-[var(--border)]/30">
                {filteredData.map((item, idx) => (
                  <tr key={idx} className="hover:bg-[var(--bg-card)]/50 transition-colors group">
                    <td className="p-5 font-bold text-[var(--text-primary)] text-sm tracking-tight">{item.name}</td>
                    <td className="p-5">
                      <span className="text-[10px] text-[var(--text-muted)] uppercase tracking-widest font-black">{item.category}</span>
                    </td>
                    <td className="p-5 text-xs text-[var(--text-muted)] max-w-md italic leading-relaxed">{item.description}</td>
        ...
        {filteredData.length === 0 && (
          <div className="py-32 text-center border border-dashed border-[var(--border)]/50 rounded-none">
            <div className="opacity-10 text-4xl uppercase tracking-[0.6em] font-light mb-4 italic">No Matches Found</div>
            <p className="text-[10px] text-[var(--text-muted)] uppercase font-bold tracking-widest">The current query returned an empty dataset from the cloud sheet.</p>
          </div>
        )}
      </div>

      {/* System Status Footer */}
      <div className="bg-[var(--bg-card)]/30 p-4 border border-[var(--border)] flex items-center justify-between text-[9px] text-[var(--text-muted)] uppercase tracking-widest font-bold">
