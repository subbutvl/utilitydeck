...
const SHEET_URL =
  "https://docs.google.com/spreadsheets/d/1WQZvOZIh4FHyTGKj_ukjYRyksBZpRqith-5by6pRvIU/export?format=csv";
  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        if (!response.ok)
          throw new Error("Handshake with Cloud Storage failed.");
        const text = await response.text();
        const rows = parseCSV(text);
        const parsed: Resource[] = rows
          .slice(1)
        ...
        setError(
          "Synchronization Error: The cloud database could not be reached."
        );
      } finally {
        setLoading(false);
      }
    ...
    return (
      <div className="h-full flex items-center justify-center">
        <div className="text-center space-y-4">
          ...
          <p className="text-[10px] uppercase font-bold text-neutral-600 tracking-[0.4em]">
    ...
    return (
      <div className="h-full flex items-center justify-center p-10">
        <div className="max-w-md text-center space-y-6">
          ...
          <p className="text-xs text-neutral-500 leading-relaxed font-bold uppercase tracking-widest">
            {error}
          </p>
          <button
          ...
          <button
            ...
            className={`px-5 py-2 border shrink-0 text-[10px] uppercase font-bold tracking-[0.2em] transition-all ${
              activeCategory === cat
                ...
                : "bg-neutral-900/50 text-neutral-500 border-neutral-800 hover:border-neutral-600"
            }`}
          >
            {cat}
          </button>
      ...
      <div className="flex-1 overflow-y-auto pr-1 pb-24 custom-scrollbar">
            ...
            {filteredData.map((item, idx) => (
              ...
              >
                ...
                <div className="space-y-2">
                  ...
                  <h3 className="text-lg font-bold text-white group-hover:text-indigo-400 transition-colors tracking-tight leading-tight">
                    {item.name}
                  </h3>
                </div>
                ...
                <div className="mt-auto pt-6 flex flex-wrap gap-2">
                  {item.tags.map((tag) => (
                    <span
                      key={tag}
                      className="text-[9px] text-neutral-600 bg-black/50 px-2 py-0.5 border border-neutral-900 font-mono uppercase tracking-tighter group-hover:border-neutral-800 transition-colors"
                    >
                      {tag}
                    </span>
                  ))}
            ...
            <table className="w-full text-left border-collapse">
              ...
              <tbody className="divide-y divide-[var(--border)]/30">
                {filteredData.map((item, idx) => (
                  ...
                  >
                    <td className="p-5 font-bold text-neutral-200 text-sm tracking-tight">
                      {item.name}
                    </td>
                    <td className="p-5">
                    ...
                    <td className="p-5 text-xs text-neutral-500 max-w-md italic leading-relaxed">
                      {item.description}
                    </td>
                    <td className="p-5 text-right">
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="inline-block text-indigo-400 hover:text-indigo-300 text-[10px] font-bold uppercase tracking-widest border-b border-indigo-500/20 pb-0.5 hover:border-indigo-400 transition-all"
                      >
                        Launch Resource
                      </a>
                    </td>
        ...
        {filteredData.length === 0 && (
          <div className="py-32 text-center border border-dashed border-neutral-800/50 rounded-none">
            <div className="opacity-10 text-4xl uppercase tracking-[0.6em] font-light mb-4 italic">
              No Matches Found
            </div>
            <p className="text-[10px] text-neutral-600 uppercase font-bold tracking-widest">
              The current query returned an empty dataset from the cloud sheet.
            </p>
          </div>
        )}
      </div>
