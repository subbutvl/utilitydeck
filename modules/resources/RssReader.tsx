      ...
      <div className="flex-1 overflow-y-auto pr-1 pb-20 custom-scrollbar">
        {loading ? (
          <div className="h-full flex flex-col items-center justify-center gap-4 py-40">
             ...
             <span className="text-[10px] uppercase font-bold text-neutral-600 tracking-[0.3em] animate-pulse">Syncing Feed Matrix...</span>
          </div>
        ) : error ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {items.map((item, idx) => (
              ...
              >
                ...
                <div className="text-xs text-neutral-500 line-clamp-3 leading-relaxed" dangerouslySetInnerHTML={{ __html: item.description.replace(/<[^>]*>?/gm, '') }} />

                <div className="mt-auto pt-4 border-t border-neutral-900 flex justify-between items-center">
                   <span className="text-[9px] uppercase font-bold text-neutral-700 tracking-widest">Read Summary</span>
           ...
           <div className="relative w-full max-w-3xl max-h-[80vh] bg-[#0a0a0a] border border-neutral-800 flex flex-col shadow-2xl overflow-hidden">
              <div className="p-6 border-b border-neutral-800 flex justify-between items-center shrink-0">
                 <div className="flex items-center gap-3">
                    ...
                    <span className="text-[10px] font-mono text-neutral-600">/ NEWS DATA /</span>
                 </div>
                 <button onClick={() => setSelectedItem(null)} className="text-neutral-600 hover:text-white transition-colors">
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-8 custom-scrollbar">
                 <div className="space-y-4">
                    <div className="text-[10px] uppercase font-bold text-neutral-600 tracking-widest">{new Date(selectedItem.pubDate).toUTCString()}</div>
                    <h2 className="text-3xl font-bold text-white leading-tight tracking-tight">{selectedItem.title}</h2>
                    {selectedItem.author && <div className="text-xs text-neutral-500 italic">By {selectedItem.author}</div>}
                 </div>
